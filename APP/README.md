...
c499 is the module that contains the application, and c499_test is the module that contains the testing code for c499
...
Lets take a look at the frontend testing code, where we are supposed to run the frontend **without the backend**. How can we do it? We use a method called mocking. Lets take a look at the file `test_registraion.py` under the `c499_test/frontend` folder:
...
class FrontEndHomePageTest(BaseCase):
    ...
    @patch('c499.backend.get_user', return_value=test_user)
    @patch('c499.backend.get_all_tickets', return_value=test_tickets)
